                    ...
                    <form method="post" action="{{ route('quiz.update', $quiz->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="f-inter flex flex-col mb-6">
                            ...
                            <input name="image"  type="file"
                                class="bg-[#EEF6F1] border border-[#F3FFF2] py-4 px-3 text-[16px] font-[400] text-[#323232] rounded-[5px] placeholder-[#323232]">
                            @if($quiz->image)
                                <img src="{{ asset($quiz->image) }}" width="100px">
                            @endif
                            ...
                            <div id="ans" class="mt-5">
                                @foreach ($quizAns as $index => $quizAn)
                                   <div class="mt-5">
                                        <div class="ansdiv">
                                        <label
                                            class="f-inter mt-5 text-[16px] text-[#0D2927] mb-1 font-bold">Answer</label>
                                        <div class="flex w-full">

                                            <input required type="text" name="answers[]"
                                                value="{{ $quizAn->ans }}"
                                                class="w-[95%] bg-[#EEF6F1] border border-[#F3FFF2] py-4 px-3 text-[16px] font-[400] text-[#323232] rounded-[5px] placeholder-[#323232]">

                                         <div class="mt-[15px] col-span-2 ml-[15px] text-end">
                                            <button class="minusdiv" type="button"><svg
                                                style="width:15px;height:15px" xmlns="http://www.w3.org/2000/svg"
                                                viewBox="0 0 448 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.-->
                                                <path fill="#ff0000"
                                       ...
                                       <div class="mt-3">
                                        <label
                                        class="f-inter mt-3 text-[16px] text-[#0D2927] mb-1 font-bold">Tag</label>

                                    @php
                                        $selectedTags = explode(',', $quizAn->advertisement_id);
                                    @endphp
                                    <select required name="tag[{{ $index }}][]" multiple
                                    class="tags-select w-[95%] bg-[#EEF6F1] border border-[#F3FFF2] py-1 px-3 text-[16px] font-[400] text-[#323232] rounded-[5px] placeholder-[#323232]">
                                    <option value="" disabled>Select a tag</option>
                                    @foreach ($advertiseMents as $advertiseMent)
                                        <option value="{{ $advertiseMent->id }}"
                                            {{ in_array($advertiseMent->id, old('tag.' . $index, $selectedTags ?? [])) ? 'selected' : '' }}>
                                            {{ $advertiseMent->tag }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        {{--
                        <div class="grid grid-cols-12 border-b-2 border-black-900 mb-2">
                            <div class="col-span-7"><label class="f-inter text-[16px] text-[#0D2927] mb-1 font-bold"
                                    for="">Answer</label></div>
                            <div class="col-span-3"><label class="f-inter text-[16px] text-[#0D2927] mb-1 font-bold"
                                    for="">Tag</label></div>
                            <div class="col-span-2 text-end"> <button id="addnew" type="button">
                                    <svg style="width: 15px;height:15px" xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 448 512"><!--!Font Awesome Free 6.7.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2025 Fonticons, Inc.-->
                                        <path style="fill:#228B22"
                                            d="M256 80c0-17.7-14.3-32-32-32s-32 14.3-32 32l0 144L48 224c-17.7 0-32 14.3-32 32s14.3 32 32 32l144 0 0 144c0 17.7 14.3 32 32 32s32-14.3 32-32l0-144 144 0c17.7 0 32-14.3 32-32s-14.3-32-32-32l-144 0 0-144z" />
                        ...
                        <div id="ans">
                            @foreach ($quizAns as $index => $quizAn)
                                <div class="grid grid-cols-12  gap-2 ansdiv mb-2">
                                    <div class="col-span-7">
                                        <input required type="text" name="answers[]" value="{{ $quizAn->ans }}"
